...
<script lang="ts">
    ...
    const disabledNewSortOrders: number[] = $derived.by(() => {
    ...
    $effect(() => {
        // disabled options aren't deselected automatically, and a previous version
        // may have left an invalid index behind. The default is never disabled.
        if (
            disabledNewSortOrders.includes($config.newCardSortOrder) ||
            !($config.newCardSortOrder in SortOrder)
        ) {
