...
fn build_generated_sources(build: &mut Build) -> Result<()> {
    ...
    // qt/_aqt/colors.py, props.py, and theme_schema.json are generated by the
    // "qt:aqt:sass_vars" action registered in web.rs's build_sass(), so that
    // ts/lib/sass/_root-vars.scss can consume the same generated tokens partial.
    ...
    for scss in scss_files {
        ...
        build.add_action(
            ...
            CompileSass {
                ...
                deps: inputs![":sass"],
                load_paths: vec![".", "node_modules", "$builddir/ts/lib/sass"],
            },
        )?;
    }
